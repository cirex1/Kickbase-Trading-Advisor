...
# from IPython.display import display   # in Actions nutzlos; auskommentiert, um Issues zu vermeiden
...
# --------- Output suitable for GitHub Actions (Logs + CSV artefacts) ---------
print("\n=== Manager Budgets ===")
print(manager_budgets_df.to_string(index=False))
manager_budgets_df.to_csv("budgets.csv", index=False)

print("\n=== Market Recommendations (Top 25) ===")
print(market_recommendations_df.head(25).to_string(index=False))
market_recommendations_df.to_csv("market_recos.csv", index=False)

print("\n=== Squad Recommendations (Top 25) ===")
print(squad_recommendations_df.head(25).to_string(index=False))
squad_recommendations_df.to_csv("squad_recos.csv", index=False)

# --------- Email (optional, fehlertolerant) ---------
try:
    if email and os.getenv("EMAIL_PASS"):
    ...
    # Run soll nicht fehlschlagen, wenn Mail nicht klappt.
    print(f"WARNING: send_mail failed: {e}")
